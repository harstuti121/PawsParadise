...
export default function Services() {
  ...
  const handleServiceClick = (service) => {
    if (service.title === 'Online pet store') {
      navigate('/shop');  // Navigate to the Shop component when Online Pet Store is clicked
    }else if (service.title === 'Consult a vet online') {
      navigate('/book'); // Navigate to the Book component when Consult a vet online is clicked
    }else if (service.title === 'Book Grooming Sessions') {
      navigate('/grooming'); // Navigate to the Book component when Consult a vet online is clicked
    }
  };
    ...
    <section className="py-16">
      <div className="container mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
        {services.map(service => (
          <div
            key={service.id}
            className="bg-white shadow-lg p-6 rounded-lg text-center cursor-pointer"
