      ...
      <section className="py-10 bg-gray-100">
        <div className="container mx-auto">
          <div className="flex justify-center flex-wrap">
            {DogFoodData.map((product) => (
              <div key={product.id} className="bg-white shadow-lg rounded-lg p-6 hover:shadow-xl transition-shadow duration-300 m-3" style={{ flex: '0 0 calc(80% / 4)', maxWidth: 'calc(80% / 4)' }}>
                <img src={product.image} alt={product.title} className="mb-2 rounded-lg w-full h-48 object-cover" />
                <h3 className="text-xl font-bold mb-2">{product.title}</h3>
                <p className="text-gray-600 mb-2">{product.price}</p>
                ...
                <div className="flex justify-start mb-2">
                  {[...Array(5)].map((_, index) => (
                    <CiStar key={index} className={index < product.rating ? 'text-yellow-400' : 'text-gray-300'} />
                  ))}
                </div>
